...
3. **Совместимость с HTMX**:
   ...
   - **Критичные HTML ID элементов**: `#mailboxes`, `#migrate-progress`, `#check-progress`, `#reconcile-progress`, `#preview`, `#test-{endpoint_id}`, `#log-view`, `#log-mailbox`, `#log-download`, `#row-folders-{mailbox_id}`, `#folders-{mailbox_id}`. При модификации шаблонов эти ID **должны сохраняться**.
...
5. **Прогресс переноса устроен так, и это осознанно**:
   - **отдельного бара «текущий ящик» вверху нет**. Он показывал один ящик из
     нескольких работающих параллельно, и остальные оставались невидимыми.
     Сверху — только общий бар по проекту;
   - прогресс каждого ящика живёт **в его строке таблицы**: полоса плюс
     «перенесено / всего писем · объём · скорость»;
   - строка разворачивается по кнопке `[data-mailbox-toggle="{id}"]`, содержимое
     грузится в `#folders-{id}`, сама строка — `#row-folders-{id}`;
   - таблица целиком перерисовывается каждые 5 секунд, поэтому раскрытые строки
     помнит `window.__openMailboxes` и восстанавливает скрипт в конце
     `partials/mailbox_table.html`. **Без него развёрнутый ящик схлопывается сам.**

6. **Поля, зависящие от выбранного режима** (скрываются через атрибут `hidden`):
