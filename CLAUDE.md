...
5. **Фоновое обновление идёт без перерисовки разметки**:
   - страница раз в 2 секунды забирает `/projects/{id}/live` — это **только цифры
     и готовые строки для показа**, без HTML;
   - значения меняются на месте по атрибутам `data-live="bar|counts|status"`
     внутри `[data-mailbox-row]` и `data-live="migration-*"` / `"check-*"`
     в карточках прогресса. Форматирование чисел делает сервер, чтобы оно не
     разъехалось с серверным;
   - **периодического `hx-trigger="every Ns"` на таблице и карточках больше нет.**
     Полная перерисовка происходит по событию `refresh-structure`, и только когда
     меняется состав строк или состояние прогона (за этим следит поле `structure`
     в ответе). Если вернуть периодический опрос, снова начнёт сбиваться скролл,
     выделение текста и фокус в поле поиска, а развёрнутые строки — моргать;
   - опрос останавливается, когда вкладка не видна.

6. **Прогресс переноса устроен так, и это осознанно**:
...
7. **Поля, зависящие от выбранного режима** (скрываются через атрибут `hidden`):
